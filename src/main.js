...
import { createRenderer, updateRendererSize, checkWebGL } from './core/renderer.js';
...
class App {
  ...
  init() {
    ...
    this.isWebGL = !this.renderer._isFallback;
    console.log('WebGL available:', this.isWebGL);
    ...
    if (this.isWebGL) {
      try {
        ...
        console.warn('EffectComposer failed:', e);
        this.useComposer = false;
      }
      ...
      console.log('Using Canvas 2D fallback - no Three.js scene objects needed');
      this.controls = createControls(this.camera, this.renderer);
    }
  ...
  setupModeUI() {
    ...
    modeItems.forEach(item => {
      item.addEventListener('click', (e) => {
        ...
        if (modeMap[mode] && this.earthGroup) {
  ...
  updatePlayerUI() {
    ...
    if (this.isPlaying) {
      ...
      if (fpMusicCover) fpMusicCover.classList.add('playing');
    } else {
      fpPlayerWrap.classList.remove('playing');
      if (fpMusicCover) fpMusicCover.classList.remove('playing');
    }
  }
    ...
    Object.entries(sliderMap).forEach(([sliderId, config]) => {
      ...
      slider.addEventListener('input', () => {
        ...
        if (config.type === 'shader') {
          ...
          const bw = wSlider ? parseFloat(wSlider.value) : cfg.beamWidth;
          ...
          setBeamDimensions(bw, minH, maxH);
        }
      });
    });
  }
